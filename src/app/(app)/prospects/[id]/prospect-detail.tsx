...
import type { Channel, ReplyCategory } from "@/lib/types";
...
export function ProspectDetail({ id }: { id: string }) {
  ...
  function generate(channel: Channel) { generateMessage(id, channel); setSaved(`Brouillon ${channel} créé`); }
  function saveNotes(formData: FormData) { updateProspect(id, { notes: String(formData.get("notes")), qualificationReason: String(formData.get("qualificationReason")) }); setSaved("Fiche enregistrée"); }
  function reply(formData: FormData) { recordReply(id, String(formData.get("category")) as ReplyCategory, String(formData.get("replyText"))); setReplyOpen(false); setSaved("Réponse enregistrée · relances stoppées"); }
  function win(formData: FormData) { markWon(id, Number(formData.get("amount")), Number(formData.get("paidAmount"))); setDealOpen(false); setSaved("Vente enregistrée"); }
  ...
  return <>
    ...
    <Card className="profile-hero section">
      <div><p className="eyebrow">{prospect.city}, {prospect.state}</p><h1>{prospect.businessName}</h1><div className="button-row"><Badge tone={prospect.status === "DO_NOT_CONTACT" ? "red" : prospect.status === "WON" || prospect.status === "INTERESTED" ? "green" : "neutral"}>{prospect.status}</Badge><Badge>{prospect.category}</Badge>{prospect.analysis?.demo ? <Badge tone="blue">Demo AI result</Badge> : null}</div></div>
    ...
    {tab === "messages" ? <div className="list section">{messages.map((message) => <Card className="profile-section" key={message.id}><div className="section-heading"><div><Badge>{message.channel}</Badge> <Badge tone={message.status === "APPROVED" ? "green" : "neutral"}>{message.status}</Badge></div><small className="muted">{message.kind.replaceAll("_", " ")}</small></div>{message.subject ? <h3>{message.subject}</h3> : null}<p>{message.body}</p></Card>)}{!messages.length ? <Card><EmptyState title="Aucun message" description="Générez votre premier message depuis l’onglet Résumé." /></Card> : null}</div> : null}
