...
const websiteTypeLabel = {
  dedicated: "Site dédié",
  link_in_bio: "Page de liens",
...
export function ProspectDetail({ id }: { id: string }) {
  ...
  async function generate(channel: Channel) { const messageId = await generateMessage(id, channel); if (messageId) setSaved(`Brouillon ${channel} créé`); }
  ...
  return <>
    ...
    {tab === "summary" ? <div className="grid grid-2 section">
      <Card className="profile-section"><div className="section-heading"><h2>Pourquoi le contacter</h2><button className="button small-button" disabled={prospect.status === "DO_NOT_CONTACT" || prospect.enrichment?.prequalification === "reject"} onClick={async () => { const analyzed = await analyzeProspect(id, { forceOpenAI: manualLowAnalysis }); if (analyzed) setSaved("Analyse mise à jour"); }}><Bot size={15} />{manualLowAnalysis ? "Analyser avec Terra" : "Analyser"}</button></div><p>{prospect.qualificationReason}</p>{prospect.analysis ? <div className="detail-list"><div className="detail-row"><span>Problème principal</span><strong>{prospect.analysis.mainProblem}</strong></div><div className="detail-row"><span>Meilleur canal</span><strong>{prospect.analysis.bestChannel}</strong></div><div className="detail-row"><span>Angle conseillé</span><strong>{prospect.analysis.salesAngle}</strong></div>{prospect.analysis.webSources?.length ? <div className="detail-row"><span>Sources web OpenAI</span><strong>{prospect.analysis.webSources.map((source) => <a href={source.url} target="_blank" rel="noreferrer" key={source.url}>{source.title}</a>)}</strong></div> : null}</div> : <p className="muted">{manualLowAnalysis ? "Préqualification faible : aucune analyse OpenAI automatique. Vous pouvez la lancer explicitement." : "Lancez l’analyse pour obtenir un rapport structuré."}</p>}</Card>
      <Card className="profile-section"><h2>Détail du score</h2>{prospect.scoreBreakdown?.length ? <div className="detail-list">{prospect.scoreBreakdown.map((rule) => <div className="detail-row" key={rule.code}><span>{rule.label}</span><strong>{rule.points > 0 ? "+" : ""}{rule.points}</strong></div>)}</div> : <p className="muted">Aucune règle appliquée. Les faits inconnus ne donnent ni bonus ni malus.</p>}</Card>
    ...
    {tab === "presence" ? <div className="grid grid-2 section">
      <Card className="profile-section"><h2>Site web</h2><div className="detail-list"><div className="detail-row"><span>Type</span><strong>{prospect.websiteType ? websiteTypeLabel[prospect.websiteType] : "Inconnu"}</strong></div><div className="detail-row"><span>Site dédié</span><strong>{String(prospect.hasWebsite)}</strong></div><div className="detail-row"><span>Qualité</span><strong>{prospect.websiteQualityScore ?? "unknown"}{prospect.websiteQualityScore ? "/100" : ""}</strong></div><div className="detail-row"><span>Mobile friendly</span><strong>{String(prospect.websiteMobileFriendly)}</strong></div><div className="detail-row"><span>HTTPS</span><strong>{String(prospect.websiteHttps)}</strong></div></div><p className="muted small">{prospect.websiteNotes ?? "Aucune note."}</p></Card>
      <Card className="profile-section"><h2>Présence locale & sociale</h2><div className="detail-list"><div className="detail-row"><span>Adresse</span><strong>{prospect.address ?? "Inconnue"}</strong></div><div className="detail-row"><span>Téléphone</span><strong>{prospect.phone ?? "Inconnu"}</strong></div><div className="detail-row"><span>Email</span><strong>{prospect.email ?? "Inconnu"}</strong></div><div className="detail-row"><span>Instagram actif</span><strong>{String(prospect.instagramActive)}</strong></div><div className="detail-row"><span>Facebook actif</span><strong>{String(prospect.facebookActive)}</strong></div><div className="detail-row"><span>Avis vérifiés</span><strong>{prospect.rating !== undefined || prospect.reviewCount !== undefined ? <>{prospect.reviewCount !== undefined ? `${prospect.reviewCount} avis` : "Nombre inconnu"}{prospect.rating !== undefined ? ` · ${prospect.rating}/5` : ""}</> : "Inconnus"}</strong></div><div className="detail-row"><span>Entreprise indépendante</span><strong>{String(prospect.independentBusiness)}</strong></div><div className="detail-row"><span>Franchise probable</span><strong>{String(prospect.likelyFranchise)}</strong></div></div></Card>
      <Card className="profile-section"><h2>Enrichissement & provenance</h2>{prospect.enrichment ? <><div className="detail-list">{prospect.enrichment.providers.map((provider) => <div className="detail-row" key={provider.provider}><span>{provider.provider.replaceAll("_", " ")}</span><strong>{provider.status}{provider.cached ? " · cache" : ""}</strong></div>)}{prospect.enrichment.prequalification ? <div className="detail-row"><span>Préqualification</span><strong>{prospect.enrichment.prequalification}</strong></div> : null}<div className="detail-row"><span>Conflits conservés</span><strong>{prospect.enrichment.conflicts.length}</strong></div></div><h3>Faits externes</h3><div className="detail-list">{prospect.enrichment.evidence.filter((item) => item.source.kind !== "manual").slice(0, 20).map((item, index) => <div className="detail-row" key={`${item.source.provider}-${item.field}-${index}`}><span>{item.field} · {item.source.label}</span><strong>{item.source.url ? <a href={item.source.url} target="_blank" rel="noreferrer">{evidenceValue(item.value)}</a> : evidenceValue(item.value)}</strong></div>)}</div></> : <p className="muted">Aucun enrichissement enregistré.</p>}</Card>
    </div> : null}
